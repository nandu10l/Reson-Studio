/**
 * SampleLibrary — Loads real instrument samples from the Tonejs-Instruments CDN.
 *
 * Based on: https://github.com/nbrosowsky/tonejs-instruments
 * Samples are CC-BY 3.0 licensed.
 *
 * Usage:
 *   import { loadInstrument, INSTRUMENT_LIST } from './SampleLibrary';
 *   const sampler = loadInstrument('piano', { onload: () => console.log('ready') });
...
// ─── Sample Maps ─────────────────────────────────────────────────────────────
// Each key is a note name, each value is the filename on CDN.
...
const SAMPLES = {
    ...
    'contrabass': {
        'A2': 'A2.mp3', 'A#1': 'As1.mp3',
        'B3': 'B3.mp3',
        'C2': 'C2.mp3', 'C#3': 'Cs3.mp3',
        'D2': 'D2.mp3', 'E2': 'E2.mp3', 'E3': 'E3.mp3',
    ...
    'guitar-acoustic': {
        ...
        'D#2': 'Ds2.mp3', 'D#3': 'Ds3.mp3', 'D#4': 'Ds3.mp3',
        'E2': 'E2.mp3', 'E3': 'E3.mp3', 'E4': 'E4.mp3',
        'F2': 'F2.mp3', 'F3': 'F3.mp3', 'F4': 'F4.mp3',
...
/**
 * List of all available instrument names.
 */
export const INSTRUMENT_LIST = Object.keys(SAMPLES);

/**
 * Load a Tone.Sampler for the given instrument.
 * @param {string} instrumentName — one of INSTRUMENT_LIST
 * @param {object} [options]
 * @param {function} [options.onload]  — callback when all samples are ready
 * @param {function} [options.onerror] — callback on load failure
 * @returns {Tone.Sampler}
 */
export function loadInstrument(instrumentName, options = {}) {
    const sampleMap = SAMPLES[instrumentName];
    if (!sampleMap) {
        console.warn(`SampleLibrary: Unknown instrument "${instrumentName}"`);
        return null;
    }

    return new Tone.Sampler({
        urls: sampleMap,
        baseUrl: `${BASE_URL}${instrumentName}/`,
        release: 1,
        onload: options.onload || (() => {
            console.log(`SampleLibrary: ${instrumentName} loaded`);
        }),
        onerror: options.onerror || ((err) => {
            console.warn(`SampleLibrary: ${instrumentName} load error:`, err);
        })
    });
}

/**
 * Get the sample map for an instrument (useful for checking available notes).
 */
export function getSampleMap(instrumentName) {
    return SAMPLES[instrumentName] || null;
}

export default { INSTRUMENT_LIST, loadInstrument, getSampleMap };
